...
#ifdef RISCV64
  // index >= 0
  __ branch(lir_cond_less, index.result(), zero_reg,
            new DeoptimizeStub(info, Deoptimization::Reason_range_check, Deoptimization::Action_make_not_entrant));
  // index < length
  __ branch(lir_cond_greaterEqual, index.result(), len,
            new DeoptimizeStub(info, Deoptimization::Reason_range_check, Deoptimization::Action_make_not_entrant));
#else
  // index >= 0
